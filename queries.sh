...
echo "$($PSQL "SELECT DISTINCT name FROM teams 
      ...
      WHERE (win.year = 2014 AND win.round = 'Eighth-Final') OR (lose.year = 2014 AND lose.round = 'Eighth-Final') 
      ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM games INNER JOIN teams ON games.winner_id=teams.team_id
                ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM teams FULL JOIN games
